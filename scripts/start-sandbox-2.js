...
class SandboxManager extends EventEmitter {
   ...
   */
  spawnSandboxProcess(id = 0) {
    let port = 8080 + id; 
    return spawn("aztec", ["start", "--sandbox", "--port", port.toString()], {
      stdio: "pipe",
    });
  }
   ...
   * @param {number} processId - Identifier for the process instance
   * @param {Function} safeResolve - Safe resolution function
   * @param {Function} safeReject - Safe rejection function
   */
  setupProcessHandlers(process, processId, safeResolve, safeReject) {
    ...
    if (this.verbose) {
      process.stdout.on("data", (data) => {
        ...
        if (output) {
          console.log(`📡 Sandbox ${processId}: ${output}`);
        }
      });
    }
    ...
    process.stderr.on("data", (data) => {
      ...
      if (output) {
        if (this.verbose) {
          console.log(`🚨 Sandbox ${processId} error: ${output}`);
        }
        ...
        if (output.includes("port is already")) {
          ...
          if (this.processes[processId]) {
            this.processes[processId].kill("SIGTERM");
          }
          this.processes = [];
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        for (let i = 0; i < this.num_instances; i++) {
          this.processes.push(this.spawnSandboxProcess(i));
        }
